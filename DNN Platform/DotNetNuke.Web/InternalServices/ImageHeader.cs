...
namespace DotNetNuke.Web.InternalServices
{
    ...
    public static class ImageHeader
    {
        ...
        private static readonly Dictionary<byte[], Func<BinaryReader, Size>> s_imageFormatDecoders = new Dictionary
        ...
        {
            int maxMagicBytesLength = s_imageFormatDecoders.Keys.OrderByDescending(x => x.Length).First().Length;
            var magicBytes = new byte[maxMagicBytesLength];
            for (var i = 0; i < maxMagicBytesLength; i += 1)
            {
                magicBytes[i] = binaryReader.ReadByte();
                foreach (var pair in s_imageFormatDecoders)
                {
