...
namespace ClientDependency.Core.CompositeFiles.Providers
{
    ...
    public class CompositeFileProcessingProvider : BaseCompositeFileProcessingProvider
    {
        ...
        private static Regex s_importCssRegex = new Regex("@import url\\((.+?)\\);?", RegexOptions.Compiled);
        ...
        private void WriteContentToStream(ref StreamWriter sw, string content, ClientDependencyType type, HttpContextBase context, bool minify)
        {
            ...
            {
                while (s_importCssRegex.IsMatch(content))
                {
                    var match = s_importCssRegex.Match(content);
