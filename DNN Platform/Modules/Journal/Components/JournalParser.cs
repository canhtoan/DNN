...
namespace DotNetNuke.Modules.Journal.Components
{
    public class JournalParser
    {
        ...
        private bool _isUnverifiedUser;
        ...
        public JournalParser(PortalSettings portalSettings, int moduleId, int profileId, int socialGroupId, UserInfo userInfo)
        {
            ...
            _url = PortalSettings.DefaultPortalAlias;
            ...
            {
                _url = HttpContext.Current.Request.Url.Host;
            }
            _url = string.Format("{0}://{1}{2}",
                                UrlUtils.IsSecureConnectionOrSslOffload(HttpContext.Current.Request) ? "https" : "http",
                                _url,
                                !HttpContext.Current.Request.Url.IsDefaultPort && !_url.Contains(":") ? ":" + HttpContext.Current.Request.Url.Port : string.Empty);
        }
        ...
        public string GetList(int currentIndex, int rows)
        {
            ...
            {
                _isAdmin = CurrentUser.IsInRole(PortalSettings.AdministratorRoleName);
            }
            _isUnverifiedUser = !CurrentUser.IsSuperUser && CurrentUser.IsInRole("Unverified Users");
            ...
            statusTemplate = Regex.Replace(statusTemplate, "\\[BaseUrl\\]", _url, RegexOptions.IgnoreCase);
            linkTemplate = Regex.Replace(linkTemplate, "\\[BaseUrl\\]", _url, RegexOptions.IgnoreCase);
            photoTemplate = Regex.Replace(photoTemplate, "\\[BaseUrl\\]", _url, RegexOptions.IgnoreCase);
            fileTemplate = Regex.Replace(fileTemplate, "\\[BaseUrl\\]", _url, RegexOptions.IgnoreCase);
            ...
            foreach (JournalItem ji in journalList)
            {
                ...
                if (CurrentUser.UserID > 0 && !_isUnverifiedUser)
                {
                ...
                if (_isAdmin || CurrentUser.UserID == ji.UserId || (ProfileId > Null.NullInteger && CurrentUser.UserID == ProfileId))
                {
        ...
        internal string GetJournalTemplate(string journalType, JournalItem ji)
        {
            ...
            template = Regex.Replace(template, "\\[BaseUrl\\]", _url, RegexOptions.IgnoreCase);
        ...
        internal string GetCommentRow(JournalItem journal, CommentInfo comment)
        {
            ...
            if (comment.UserId == CurrentUser.UserID || journal.UserId == CurrentUser.UserID || _isAdmin)
            {
        ...
        private string GetStringReplacement(JournalItem journalItem)
        {
            ...
            if (CurrentUser.UserID > 0 && SocialGroupId <= 0 && !_isUnverifiedUser)
            {
                replacement = "$1";
            }
            if (CurrentUser.UserID > 0 && journalItem.SocialGroupId > 0 && !_isUnverifiedUser)
            {
