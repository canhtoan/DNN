...
namespace DotNetNuke.Web.DDRMenu.DNNCommon
{
    ...
    {
        private static readonly Dictionary<Type, XmlSerializer> s_serialisers = new Dictionary<Type, XmlSerializer>();
        ...
        internal static XmlSerializer SerialiserFor(Type t)
        {
            ...
            if (!s_serialisers.TryGetValue(t, out result))
            {
                lock (s_serialisers)
                {
                    result = (t.Name == "MenuXml")
                                 ...
                                 : new XmlSerializer(t);
                    if (!s_serialisers.ContainsKey(t))
                    {
                        s_serialisers.Add(t, result);
                    }
                }
            }
            return result;
