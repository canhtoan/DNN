...
namespace DotNetNuke.Web.DDRMenu
{
    public class MenuBase
    {
        ...
        private Settings _menuSettings;
        ...
        private HttpContext CurrentContext { get { return _currentContext ?? (_currentContext = HttpContext.Current); } }

        private PortalSettings _hostPortalSettings;
        internal PortalSettings HostPortalSettings { get { return _hostPortalSettings ?? (_hostPortalSettings = PortalController.Instance.GetCurrentPortalSettings()); } }

        private readonly Dictionary<string, string> _nodeSelectorAliases = new Dictionary<string, string>
                                                                          {
                                                                            {"rootonly", "*,0,0"},
                                                                            {"rootchildren", "+0"},
                                                                            {"currentchildren", "."}
        ...
        {
            TemplateDef.AddTemplateArguments(_menuSettings.TemplateArguments, true);
            TemplateDef.AddClientOptions(_menuSettings.ClientOptions, true);

            if (!String.IsNullOrEmpty(_menuSettings.NodeXmlPath))
            {
                LoadNodeXml();
            }
            if (!String.IsNullOrEmpty(_menuSettings.NodeSelector))
            {
                ApplyNodeSelector();
            }
            if (!String.IsNullOrEmpty(_menuSettings.IncludeNodes))
            {
                FilterNodes(_menuSettings.IncludeNodes, false);
            }
            if (!String.IsNullOrEmpty(_menuSettings.ExcludeNodes))
            {
                FilterNodes(_menuSettings.ExcludeNodes, true);
            }
            if (String.IsNullOrEmpty(_menuSettings.NodeXmlPath) && !SkipLocalisation)
            {
                new Localiser(HostPortalSettings.PortalId).LocaliseNode(RootNode);
            }
            if (!String.IsNullOrEmpty(_menuSettings.NodeManipulator))
            {
                ApplyNodeManipulator();
            }

            var imagePathOption =
                _menuSettings.ClientOptions.Find(o => o.Name.Equals("PathImage", StringComparison.InvariantCultureIgnoreCase));
        ...
        {
            htmlWriter.Write("<!-- DDRmenu v02.00.01 - {0} template -->", _menuSettings.MenuStyle);

            UserInfo user = null;
            if (_menuSettings.IncludeContext)
            {
                user = UserController.Instance.GetCurrentUserInfo();
                user.Roles = user.Roles; // Touch roles to populate
            }

            TemplateDef.AddClientOptions(new List<ClientOption> { new ClientString("MenuStyle", _menuSettings.MenuStyle) }, false);
        ...
        private void LoadNodeXml()
        {
            ...
            RootNode = cache[_menuSettings.NodeXmlPath] as MenuNode;
            if (RootNode != null)
            {
                return;
            }

            using (var reader = XmlReader.Create(_menuSettings.NodeXmlPath))
            {
                reader.ReadToFollowing("root");
                RootNode = (MenuNode)(new XmlSerializer(typeof(MenuNode), "").Deserialize(reader));
            }
            cache.Insert(_menuSettings.NodeXmlPath, RootNode, new CacheDependency(_menuSettings.NodeXmlPath));
        }
        ...
        {
            string selector;
            if (!_nodeSelectorAliases.TryGetValue(_menuSettings.NodeSelector.ToLowerInvariant(), out selector))
            {
                selector = _menuSettings.NodeSelector;
            }
        ...
        private void ApplyNodeManipulator()
        {
            RootNode =
                new MenuNode(
                    ((INodeManipulator)Activator.CreateInstance(BuildManager.GetType(_menuSettings.NodeManipulator, true, true))).
